...
resource "aws_iam_role_policy" "dynamodb_policy" {
  name = "lb_mundocolore-orders-dynamodb-policy"
...
resource "aws_lambda_function" "orders_lambda" {
  function_name = "lb_mundocolore-orders"
...
resource "aws_api_gateway_resource" "orders_health_resource" {
  rest_api_id = data.aws_api_gateway_rest_api.gateway.id
  parent_id   = aws_api_gateway_resource.orders_resource.id
  path_part   = "health"
}

resource "aws_api_gateway_resource" "orders_health_online_resource" {
  rest_api_id = data.aws_api_gateway_rest_api.gateway.id
  parent_id   = aws_api_gateway_resource.orders_health_resource.id
  path_part   = "online"
}

resource "aws_api_gateway_resource" "orders_health_data_resource" {
  rest_api_id = data.aws_api_gateway_rest_api.gateway.id
  parent_id   = aws_api_gateway_resource.orders_health_resource.id
  path_part   = "data"
}

resource "aws_api_gateway_method" "orders_health_online_get" {
  rest_api_id   = data.aws_api_gateway_rest_api.gateway.id
  resource_id   = aws_api_gateway_resource.orders_health_online_resource.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "orders_health_online_get_integration" {
  rest_api_id             = data.aws_api_gateway_rest_api.gateway.id
  resource_id             = aws_api_gateway_resource.orders_health_online_resource.id
  http_method             = aws_api_gateway_method.orders_health_online_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.orders_lambda.invoke_arn
}

resource "aws_api_gateway_method" "orders_health_data_get" {
  rest_api_id   = data.aws_api_gateway_rest_api.gateway.id
  resource_id   = aws_api_gateway_resource.orders_health_data_resource.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "orders_health_data_get_integration" {
  rest_api_id             = data.aws_api_gateway_rest_api.gateway.id
  resource_id             = aws_api_gateway_resource.orders_health_data_resource.id
  http_method             = aws_api_gateway_method.orders_health_data_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.orders_lambda.invoke_arn
}
